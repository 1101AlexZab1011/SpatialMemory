...
from bbtoolkit.structures.tensorgroups import DirectedTensorGroup, DirectedTensor


class AbstractGenerator(WritablePickle, Copyable, ABC):
    """
    ...
    def generate_gc2pc_weights(self):
        ...
        for x in range(self.n_points_x):
            for y in range(self.n_points_y):
                ...
                for i in range(self.n_per_mod):
                    for j in range(self.n_mod):
                        gc2pc_weights[x, y, i, j] = np.max(pc*self.gc_map.fr[:, :, i, j])

        return np.reshape(gc2pc_weights, (shape[0]*shape[1], shape[2]*shape[3]))
...
class MTLGenerator(AbstractGenerator):
    ...
    def initialize_cross_weights(
        ...
        Args:
            n_neurons (Coordinates2D): Spatial coordinates of neurons.
...
class TCGenerator(AbstractGenerator):
    ...
    def __init__(
        ...
        Args:
            ...
            tr_res (float): Angular resolution for transformation circuit (TC) representation.
